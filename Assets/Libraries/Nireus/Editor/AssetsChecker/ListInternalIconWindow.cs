...
public class ListInternalIconWindow : EditorWindow
{
    ...
    void Awake()
    {
        ...
        loadIconMethodInfo =
            typeof(EditorGUIUtility).GetMethod("LoadIcon", BindingFlags.Static | BindingFlags.NonPublic);
        findTextureMethodInfo =
            typeof(EditorGUIUtility).GetMethod("FindTexture", BindingFlags.Static | BindingFlags.Public);
        ...
        titleContent = new GUIContent("InternalIcon",
            loadIconMethodInfo.Invoke(null, new object[] {"WaitSpin00"}) as Texture);
    ...
    void OnGUI()
    {
        ...
        int rowCount = (lstWindowIcons.Count + lstLoadIconParmContents.Count + lstFindTextureParmContents.Count) /
            colCount + 2;
    ...
    private float DrawList(string headerText, float offsetY, int colCount, List<GUIContent> lstGUIContent,
        bool isRemoveReturn)
    {
    ...
    private void InitWindowsIconList()
    {
        ...
        Type editorWindowTitleAttrType =
            typeof(EditorWindow).Assembly.GetType("UnityEditor.EditorWindowTitleAttribute");
            ...
            foreach (Type type in types)
            {
                ...
                for (int i = 0; i < attrs.Length; ++i)
                {
                    if (attrs[i].GetType() == editorWindowTitleAttrType)
                    {
                        ...
                        {
                            bool useTypeNameAsIconName = GetPropertyValue<bool>(editorWindowTitleAttrType, attrs[i],
    ...
    {
        Type editorResourcesUtility =
            typeof(EditorWindow).Assembly.GetType("UnityEditorInternal.EditorResourcesUtility");
        ...
        foreach (MatchCollection matchCollection in enumerable)
        {
            for (int i = 0; i < matchCollection.Count; ++i)
            {
                ...
                {
                    string resName = GetPropertyValue<string>(editorResourcesUtility, null,
                        iconName.Replace("EditorResourcesUtility.", ""));
