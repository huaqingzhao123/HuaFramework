...
namespace Nireus
{
    ...
    public sealed class SpawnPool : MonoBehaviour, IList<Transform>
    {
        ...
        private void Awake()
        {
            ...
            if (this.logMessages)
                GameDebug.Log(string.Format("SpawnPool {0}: Initializing..", this.poolName));
            ...
            for (int i = 0; i < this._perPrefabPoolOptions.Count; i++)
            {
                ...
                {
                    GameDebug.LogWarning(string.Format("Initialization Warning: Pool '{0}' " +
        ...
        {
            if (this.logMessages)
                GameDebug.Log(string.Format("SpawnPool {0}: Destroying...", this.poolName));
        ...
        public void Add(Transform instance, string prefabName, bool despawn, bool parent)
        {
            for (int i = 0; i < this._prefabPools.Count; i++)
            {
                ...
                {
                    GameDebug.LogError("Unexpected Error: PrefabPool.prefabGO is null");
                    return;
                }
            ...
            GameDebug.LogError(string.Format("SpawnPool {0}: PrefabPool {1} not found.",
                                         this.poolName,
                                         prefabName));
        ...
        public void Despawn(Transform instance)
        {
            ...
            if (!despawned)
            {
                GameDebug.LogError(string.Format("SpawnPool {0}: {1} not found in SpawnPool",
                               this.poolName,
                               instance.name));
                return;
            }
        ...
        public PrefabPool GetPrefabPool(Transform prefab)
        {
            ...
            {
                if (this._prefabPools[i].prefabGO == null)
                    GameDebug.LogError(string.Format("SpawnPool {0}: PrefabPool.prefabGO is null",
                                                 this.poolName));
    ...
    public class PrefabPool
    {
        ...
        {
            if (this.logMessages)
                GameDebug.Log(string.Format("SpawnPool {0} ({1}): Despawning '{2}'",
        ...
        {
            if (this.logMessages)
                GameDebug.Log(string.Format("SpawnPool {0} ({1}): CULLING TRIGGERED! " +
            ...
            while (this.totalCount > this.cullAbove)
            {
                ...
                for (int i = 0; i < this.cullMaxPerPass; i++)
                {
                    ...
                    if (this._despawned.Count > 0)
                    {
                        ...
                        if (this.logMessages)
                            GameDebug.Log(string.Format("SpawnPool {0} ({1}): " +
            ...
            if (this.logMessages)
                GameDebug.Log(string.Format("SpawnPool {0} ({1}): CULLING FINISHED! Stopping",
                                        this.spawnPool.poolName,
                                        this.prefab.name));
        ...
        internal Transform SpawnInstance(Vector3 pos, Quaternion rot)
        {
            ...
            else
            {
                ...
                if (this.logMessages)
                    GameDebug.Log(string.Format("SpawnPool {0} ({1}): respawning '{2}'.",
                                            this.spawnPool.poolName,
                                            this.prefab.name,
                                            inst.name));
        ...
        public Transform SpawnNew(Vector3 pos, Quaternion rot)
        {
            ...
            if (this.logMessages)
                GameDebug.Log(string.Format("SpawnPool {0} ({1}): Spawned new instance '{2}'.",
                                        this.spawnPool.poolName,
                                        this.prefab.name,
                                        inst.name));
        ...
        internal void PreloadInstances()
        {
            ...
            if (this.prefab == null)
            {
                GameDebug.LogError(string.Format("SpawnPool {0} ({1}): Prefab cannot be null.",
                                             this.spawnPool.poolName,
                                             this.prefab.name));
            ...
            {
                GameDebug.LogWarning(string.Format("SpawnPool {0} ({1}): " +
