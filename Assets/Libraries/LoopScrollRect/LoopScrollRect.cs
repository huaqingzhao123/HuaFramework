...
namespace UnityEngine.UI
{
    ...
    public abstract class LoopScrollRect : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
    {
        ...
        public int itemTypeEnd { get; private set; } = 0;
        ...
        IEnumerator ScrollToCellCoroutine(int index, float speed)
        {
            ...
            StopMovement();
            UpdatePrevData();
			Debug.LogError($"ScrollToCellCoroutine idx = {index}");
		}
		...
		public void AimAtCell(int index)
		{
			int bottom_index = totalCount - (index + m_Content.childCount - 1);
			if (bottom_index < 0)
			{
				RefillCellsFromEnd();
			}
			...
			{
				RefillCellsFromStart(bottom_index);
			}
		}

		public void RefillCellsFromStart(int offset = 0)
		{
