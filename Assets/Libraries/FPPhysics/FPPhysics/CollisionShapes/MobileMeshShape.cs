...
namespace FPPhysics.CollisionShapes
{
    ...
    public class MobileMeshShape : EntityShape
    {
        ...
        /// </summary>
        public static Fix64 MeshHitUniquenessThreshold = Fix64.C1em5;
        ...
        void ComputeSolidSidedness()
        {
            ...
            ray.Direction = (vA + vB + vC) / Fix64.C3;
        ...
        TriangleSidedness ComputeSolidSidednessHelper(Ray ray)
        {
            ...
            if (triangleMesh.Tree.GetOverlaps(ray, hitList))
            {
                ...
                if (hits.Count % 2 == 0)
                {
                    ...
                    if (Vector3.Dot(normal, ray.Direction) < Fix64.C0)
        ...
        private ShapeDistributionInformation ComputeVolumeDistribution(TransformableMeshData data)
        {
            ...
            if (solidity == MobileMeshSolidity.Solid)
            {
                ...
                if (shapeInformation.Volume > Fix64.C0)
            ...
            for (int i = 0; i < data.indices.Length; i += 3)
            {
                ...
                Fix64 perVertexWeight = weight * Fix64.OneThird;
            ...
            Matrix3x3.Multiply(ref shapeInformation.VolumeDistribution, Fix64.C1 / (Fix64.C2 * totalWeight), out shapeInformation.VolumeDistribution);

            //Move the inertia tensor into position according to the center.
            Matrix3x3 additionalInertia;
            InertiaHelper.GetPointContribution(Fix64.C0p5, ref Toolbox.ZeroVector, ref shapeInformation.Center, out additionalInertia);
            Matrix3x3.Subtract(ref shapeInformation.VolumeDistribution, ref additionalInertia, out shapeInformation.VolumeDistribution);

            shapeInformation.Volume = Fix64.C0;
