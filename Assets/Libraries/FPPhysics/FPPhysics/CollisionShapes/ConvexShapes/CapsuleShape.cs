...
namespace FPPhysics.CollisionShapes.ConvexShapes
{
    ...
    public class CapsuleShape : ConvexShape
    {
        ...
        public Fix64 Length
        {
            get
            {
                return halfLength * Fix64.C2;
            }
            set
            {
                halfLength = value * Fix64.C0p5;
                OnShapeChanged();
            }
        }
        ...
        {
            UpdateConvexShapeInfo(ComputeDescription(halfLength * Fix64.C2, Radius));
            base.OnShapeChanged();
        }
        ...
        public static ConvexShapeDescription ComputeDescription(Fix64 length, Fix64 radius)
        {
            ...
            description.EntityShapeVolume.Volume = MathHelper.Pi * radius * radius * length + Fix64.FourThirds * MathHelper.Pi * radius * radius * radius;

            description.EntityShapeVolume.VolumeDistribution = new Matrix3x3();
            Fix64 effectiveLength = length + radius / Fix64.C2; //This is a cylindrical inertia tensor. Approximate.
            Fix64 diagValue = Fix64.C0p0833333333 * effectiveLength * effectiveLength + Fix64.C0p25 * radius * radius;
            description.EntityShapeVolume.VolumeDistribution.M11 = diagValue;
            description.EntityShapeVolume.VolumeDistribution.M22 = Fix64.C0p5 * radius * radius;
            description.EntityShapeVolume.VolumeDistribution.M33 = diagValue;

            description.MaximumRadius = length * Fix64.C0p5 + radius;
        ...
        {
            if (direction.y > Fix64.C0)
                extremePoint = new Vector3(Fix64.C0, halfLength, Fix64.C0);
            else if (direction.y < Fix64.C0)
                extremePoint = new Vector3(Fix64.C0, -halfLength, Fix64.C0);
            else
        ...
        public override bool RayTest(ref Ray ray, ref RigidTransform transform, Fix64 maximumLength, out RayHit hit)
        {
            ...
            if (localRay.Position.y >= -halfLength && localRay.Position.y <= halfLength && localRay.Position.x * localRay.Position.x + localRay.Position.z * localRay.Position.z <= collisionMargin * collisionMargin)
            {
                ...
                hit.Normal = new Vector3(hit.Location.x, Fix64.C0, hit.Location.z);
                Fix64 normalLengthSquared = hit.Normal.LengthSquared();
                if (normalLengthSquared > Fix64.C1em9)
            ...
            Fix64 planeDirectionLengthSquared = planeDirection.sqrMagnitude;
            ...
            Fix64 dot = Vector2.Dot(planeDirection, planeOrigin);
            ...
            Fix64 squaredDistance = closestPoint.sqrMagnitude;
            ...
            Fix64 backwardsDistance = collisionMargin * Fix64.Sqrt(Fix64.C1 - squaredDistance / (collisionMargin * collisionMargin));
            ...
            var spherePosition = new Vector3(Fix64.C0, halfLength, Fix64.C0);
            ...
            //We're above the capsule, so cast a ray against the upper sphere.
            //We don't have to worry about it hitting the bottom of the sphere since it would have hit the cylinder portion first.
            spherePosition = new Vector3(Fix64.C0, -halfLength, Fix64.C0);
            if (Toolbox.RayCastSphere(ref localRay, ref spherePosition, collisionMargin, maximumLength, out hit))
            {
                //Pull the hit into world space.
                Quaternion.Transform(ref hit.Normal, ref transform.Orientation, out hit.Normal);
