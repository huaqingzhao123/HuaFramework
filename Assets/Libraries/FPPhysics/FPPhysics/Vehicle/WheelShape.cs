...
namespace FPPhysics.Vehicle
{
    ...
    public abstract class WheelShape : ICollisionRulesOwner
    {
        ...
        /// </summary>
        protected internal Box detector = new Box(Vector3.zero, Fix64.C0, Fix64.C0, Fix64.C0);

        protected internal Matrix4x4 localGraphicTransform;
        ...
        internal Matrix4x4 steeringTransform;
        protected internal Wheel wheel;

        protected internal Matrix4x4 worldTransform;
        ...
        /// </summary>
        public Matrix4x4 LocalGraphicTransform
        {
        ...
        internal void UpdateSpin(Fix64 dt)
        {
            ...
            else if (wheel.HasSupport && wheel.brake.IsBraking && FreezeWheelsWhileBraking)
            {
                ...
                Fix64 deceleratedValue = Fix64.C0;
                if (spinVelocity > Fix64.C0)
                    deceleratedValue = MathHelper.Max(spinVelocity - brakeFreezeWheelDeceleration * dt, Fix64.C0);
                else if (spinVelocity < Fix64.C0)
                    deceleratedValue = MathHelper.Min(spinVelocity + brakeFreezeWheelDeceleration * dt, Fix64.C0);
            ...
            else if (!wheel.HasSupport && wheel.drivingMotor.TargetSpeed != Fix64.C0)
            {
            ...
            else if (!wheel.HasSupport && wheel.Brake.IsBraking)
            {
                ...
                if (spinVelocity > Fix64.C0)
                    spinVelocity = MathHelper.Max(spinVelocity - brakeFreezeWheelDeceleration * dt, Fix64.C0);
                else if (spinVelocity < Fix64.C0)
                    spinVelocity = MathHelper.Min(spinVelocity + brakeFreezeWheelDeceleration * dt, Fix64.C0);
            }
            else if (!wheel.HasSupport)
            {
                //Just idly slowing down.
                if (spinVelocity > Fix64.C0)
                    spinVelocity = MathHelper.Max(spinVelocity - airborneWheelDeceleration * dt, Fix64.C0);
                else if (spinVelocity < Fix64.C0)
                    spinVelocity = MathHelper.Min(spinVelocity + airborneWheelDeceleration * dt, Fix64.C0);
            }
