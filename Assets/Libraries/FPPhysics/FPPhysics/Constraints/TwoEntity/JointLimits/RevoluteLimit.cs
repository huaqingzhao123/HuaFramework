...
namespace FPPhysics.Constraints.TwoEntity.JointLimits
{
    /// <summary>
    ...
    public class RevoluteLimit : JointLimit, I2DImpulseConstraintWithError, I2DJacobianConstraint
    {
        ...
        public void GetMassMatrix(out Matrix2x2 massMatrix)
        {
            ...
            massMatrix.M12 = Fix64.C0;
            massMatrix.M21 = Fix64.C0;
        }
        ...
        public override Fix64 SolveIteration()
        {
            ...
            if (minIsActive)
            {
                ...
                accumulatedImpulse.x = MathHelper.Max(accumulatedImpulse.x + lambda, Fix64.C0);
            ...
            if (maxIsActive)
            {
                ...
                accumulatedImpulse.y = MathHelper.Max(accumulatedImpulse.y + lambda, Fix64.C0);
        ...
        public override void Update(Fix64 dt)
        {
            ...
            {
                if (planePositionMax > Fix64.C0 || planePositionMin > Fix64.C0)
                {
            ...
            else
            {
                ...
                if (planePositionMin <= Fix64.C0 && planePositionMax <= Fix64.C0)
                {
                    //Escape upward.
                ...
                else if (planePositionMin <= Fix64.C0)
                {
                    //It's quicker to escape out to the min plane than the max plane.
                    error.x = -planePositionMin;
                    error.y = Fix64.C0;
            ...
            if (minIsActive)
            {
                biasVelocity.x = MathHelper.Min(MathHelper.Max(Fix64.C0, error.x - margin) * errorReduction, maxCorrectiveVelocity);
                if (bounciness > Fix64.C0)
                {
            ...
            if (maxIsActive)
            {
                biasVelocity.y = MathHelper.Min(MathHelper.Max(Fix64.C0, error.y - margin) * errorReduction, maxCorrectiveVelocity);
                if (bounciness > Fix64.C0)
                {
                    //Find the velocity contribution from each connection
                    if (maxIsActive)
            ...
            velocityToImpulse.x = Fix64.C1 / (softness + minEntryA + minEntryB);
            velocityToImpulse.y = Fix64.C1 / (softness + maxEntryA + maxEntryB);
