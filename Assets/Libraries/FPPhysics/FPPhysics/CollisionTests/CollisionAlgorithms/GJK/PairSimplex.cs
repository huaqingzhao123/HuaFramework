...
namespace FPPhysics.CollisionTests.CollisionAlgorithms.GJK
{
    ...
    public struct PairSimplex
    {
        ...
        public void GetPointOnTriangleClosestToOrigin(out Vector3 point)
        {
            ...
            AdotAB = -AdotAB;
            AdotAC = -AdotAC;
            if (AdotAC <= Fix64.C0 && AdotAB <= Fix64.C0)
            {
                //It is A!
                State = SimplexState.Point;
                U = Fix64.C1;
            ...
            BdotAB = -BdotAB;
            BdotAC = -BdotAC;
            if (BdotAB >= Fix64.C0 && BdotAC <= BdotAB)
            {
                //It is B!
                State = SimplexState.Point;
                A = B;
            ...
            if (vc <= Fix64.C0 && AdotAB > Fix64.C0 && BdotAB < Fix64.C0)//Note > and < instead of => <=; avoids possibly division by zero
            {
            ...
            CdotAB = -CdotAB;
            CdotAC = -CdotAC;
            if (CdotAC >= Fix64.C0 && CdotAB <= CdotAC)
            {
                //It is C!
                State = SimplexState.Point;
                A = C;
            ...
            if (vb <= Fix64.C0 && AdotAC > Fix64.C0 && CdotAC < Fix64.C0)//Note > instead of >= and < instead of <=; prevents bad denominator
            {
            ...
            Fix64 d3d4;
            Fix64 d6d5;
            if (va <= Fix64.C0 && (d3d4 = BdotAC - BdotAB) > Fix64.C0 && (d6d5 = CdotAB - CdotAC) > Fix64.C0)//Note > instead of >= and < instead of <=; prevents bad denominator
            {
                //Throw away A.  C->A.
                                                   ...
                                                   ref Vector3 otherPoint, out PairSimplex simplex, out Vector3 point)
        {
            ...
            if (AdotN * ADdotN >= -Toolbox.Epsilon * errorTolerance)
            {
                ...
                simplex.W = vc * denom;
                simplex.V = vb * denom;
                simplex.U = Fix64.C1 - simplex.V - simplex.W;
                Vector3.Multiply(ref ab, simplex.V, out point);
                Vector3 acw;
        ...
        internal void VerifyContributions()
        {
            ...
            {
                case SimplexState.Point:
                    if (Vector3.Distance(SimplexA.A - SimplexB.A, A) > Fix64.C1em4)
                ...
                case SimplexState.Segment:
                    ...
                    if (Vector3.Distance(SimplexA.B - SimplexB.B, B) > Fix64.C1em4)
                        Debug.WriteLine("break.");
                    break;
                case SimplexState.Triangle:
                    if (Vector3.Distance(SimplexA.A - SimplexB.A, A) > Fix64.C1em4)
                    ...
                    if (Vector3.Distance(SimplexA.C - SimplexB.C, C) > Fix64.C1em4)
                        Debug.WriteLine("break.");
                    break;
                ...
                case SimplexState.Tetrahedron:
                    ...
                    if (Vector3.Distance(SimplexA.D - SimplexB.D, D) > Fix64.C1em4)
                        Debug.WriteLine("break.");
                    break;
            }
        }
