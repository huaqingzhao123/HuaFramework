    ...
    public static class GJKToolbox
    {
                                   ...
                                   out RayHit hit)
        {
            ...
            while (closestOffset.LengthSquared() >= Toolbox.Epsilon * simplex.GetErrorTolerance(ref ray.Position))
            {
                ...
                if (vw > Fix64.C0)
                {
                    ...
                    if (closestPointDotDirection >= Fix64.C0)
                    {
                        hit = new RayHit();
                        return false;
                    }
                                   ...
                                   out RayHit hit)
        {
            ...
            while (true)
            {
                ...
                if (hit.T > maximumLength || hit.T < Fix64.C0)
                ...
                if (iterations > 3) //Limit could be configurable.
                {
                    ...
                    return GJKToolbox.RayCast(ray, target, ref shapeTransform, maximumLength, out hit) && hit.T > Fix64.C0;
