namespace FPPhysics.CollisionTests.CollisionAlgorithms.GJK
{
    ...
    public struct SimpleSimplex
    {
        ...
        public void GetPointOnTriangleClosestToOrigin(out Vector3 point)
        {
            ...
            if (vb <= Fix64.C0 && d2 > Fix64.C0 && d6 < Fix64.C0) //Note > instead of >= and < instead of <=; prevents bad denominator
            {
            ...
            Fix64 d3d4;
            Fix64 d6d5;
            if (va <= Fix64.C0 && (d3d4 = d4 - d3) > Fix64.C0 && (d6d5 = d5 - d6) > Fix64.C0)//Note > instead of >= and < instead of <=; prevents bad denominator
            {
                //Throw away A.  C->A.
