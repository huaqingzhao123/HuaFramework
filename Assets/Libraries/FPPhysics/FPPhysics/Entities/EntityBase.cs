        ...
        IEquatable<Entity>
    {
        ...
        public Fix64 Mass
        {
            ...
            set
            {
                ...
                // if (value <= 0 || Fix64.IsNaN(value) || Fix64.IsInfinity(value))
                if (value <= Fix64.C0)
                    BecomeKinematic();
                else
                {
        ...
        public void BecomeDynamic(Fix64 mass, Matrix3x3 localInertiaTensor)
        {
            ...
            this.mass = mass;
            this.inverseMass = Fix64.C1 / mass;
        ...
        void IForceUpdateable.UpdateForForces(Fix64 dt)
        {
                ...
                (activityInformation.isSlowing || activityInformation.velocityTimeBelowLimit > activityInformation.DeactivationManager.lowVelocityTimeMinimum))
            {
                ...
                {
                    Fix64 boost = Fix64.C1 - Fix64.Sqrt(energy) / (Fix64.C2 * activityInformation.DeactivationManager.velocityLowerLimit);
            ...
            if (linear > Fix64.C0)
            {
                Vector3.Multiply(ref linearVelocity, Fix64.Pow(MathHelper.Clamp(Fix64.C1 - linear, Fix64.C0, Fix64.C1), dt), out linearVelocity);
            }
...
#else
                Vector3.Multiply(ref angularVelocity, Fix64.Pow(MathHelper.Clamp(Fix64.C1 - angular, Fix64.C0, Fix64.C1), dt), out angularVelocity);
#endif
            }

            linearDampingBoost = Fix64.C0;
            angularDampingBoost = Fix64.C0;
        ...
        void ICCDPositionUpdateable.ResetTimesOfImpact()
        {
            ...
            for (int i = 0; i < collisionInformation.pairs.Count; i++)
            {
                collisionInformation.pairs.Elements[i].timeOfImpact = Fix64.C1;
            }
        }

        void ICCDPositionUpdateable.UpdatePositionContinuously(Fix64 dt)
        {
            Fix64 minimumToi = Fix64.C1;
            for (int i = 0; i < collisionInformation.pairs.Count; i++)
            {
        ...
        void IPositionUpdateable.PreUpdatePosition(Fix64 dt)
        {
            ...
            Vector3.Multiply(ref angularVelocity, dt * Fix64.C0p5, out increment);
            var multiplier = new Quaternion(increment.x, increment.y, increment.z, Fix64.C0);
        ...
        public Fix64 AngularDamping
        {
            ...
            set
            {
                angularDamping = MathHelper.Clamp(value, Fix64.C0, Fix64.C1);
            }
        }
        ///<summary>
        ...
        public Fix64 LinearDamping
        {
            ...
            set
            {
                linearDamping = MathHelper.Clamp(value, Fix64.C0, Fix64.C1);
            }
        }
        ...
        public void ModifyLinearDamping(Fix64 damping)
        {
            ...
            Fix64 remainder = Fix64.C1 - totalDamping;
