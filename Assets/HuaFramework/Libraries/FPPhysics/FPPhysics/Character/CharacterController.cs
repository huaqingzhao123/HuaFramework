...
using FPPhysics.NarrowPhaseSystems.Pairs;
using FPPhysics.PositionUpdating;
using FPPhysics.UpdateableSystems;
using System;
...
namespace FPPhysics.Character
{
    ...
    public class CharacterController : Updateable, IBeforeSolverUpdateable
    {
        ...
        Vector3 viewDirection = new Vector3(Fix64.C0, Fix64.C0, -1);
        ...
        Fix64 jumpForceFactor = Fix64.C1;
        /// <summary>
        ...
        private Fix64 speedScale = Fix64.C1;
        /// <summary>
            ...
            )
        {
            ...
            HorizontalMotionConstraint.PositionAnchorDistanceThreshold = (Fix64)radius * Fix64.C0p25;
        ...
        void ExpandBoundingBox()
        {
            if (Body.ActivityInformation.IsActive)
            {
                ...
                var horizontalExpansionAmount = Body.CollisionInformation.Shape.CollisionMargin * Fix64.C1p1;
                Vector3 squaredDown;
        ...
        void IBeforeSolverUpdateable.Update(Fix64 dt)
        {
            ...
            try
            {
                ...
                if (SupportFinder.HasSupport && !hadSupport && verticalVelocity < Fix64.C0)
                {
                ...
                if (TryToJump)
                {
                    ...
                    if (SupportFinder.HasTraction)
                    {
                        ...
                        Fix64 velocityChange = MathHelper.Max(jumpSpeed + currentDownVelocity, Fix64.C0);
                    ...
                    else if (SupportFinder.HasSupport)
                    {
                        ...
                        //Target velocity is JumpSpeed.
                        Fix64 velocityChange = MathHelper.Max(slidingJumpSpeed - currentNormalVelocity, Fix64.C0);
        ...
        void CorrectContacts()
        {
            ...
            Fix64 minimumHeight = Body.Height * Fix64.C0p5 - margin;
