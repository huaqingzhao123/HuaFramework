...
namespace FPPhysics.Character
{
    ...
    public class HorizontalMotionConstraint : SolverUpdateable
    {
        ...
        public Vector2 MovementDirection
        {
            ...
            set
            {
                if (movementDirection.x != value.x || movementDirection.y != value.y) //Fix64ing point comparison is perfectly fine here. Any bitwise variation should go through.
                {
                    ...
                    Fix64 lengthSquared = value.sqrMagnitude;
        ...
        private Fix64 maxAccelerationForceDt;
        ...
        {
            get { return timeSinceTransition < Fix64.C0; }
        }
        ...
        private Fix64 supportForceFactor = Fix64.C1;
        ...
        private Fix64 timeSinceTransition;
        ...
        {
            isTryingToMove = movementDirection3d.LengthSquared() > Fix64.C0;
            ...
            if (MovementMode != MovementMode.Floating)
            {
                ...
                if (isTryingToMove)
                {
                    ...
                    Plane plane = new Plane(supportData.Normal, Fix64.C0);
                    Fix64 t;
            ...
            targetVelocity.x = isTryingToMove ? TargetSpeed : Fix64.C0;
            targetVelocity.y = Fix64.C0;

            //Compute the effective mass matrix.
            if (supportEntity != null && supportEntity.IsDynamic)
            {
                Fix64 m11, m22, m1221 = Fix64.C0;
                Fix64 inverseMass;
            ...
            if (!isTryingToMove && MovementMode == MovementMode.Traction && supportEntity != null)
            {
                ...
                if (timeSinceTransition >= Fix64.C0 && timeSinceTransition < timeUntilPositionAnchor)
                ...
                if (timeSinceTransition < Fix64.C0)
                {
                    ...
                    else
                    {
                        ...
                        Vector2.Multiply(ref positionCorrectionBias, Fix64.C0p2 / dt, out positionCorrectionBias);
                    }
                }
            }
            else
        ...
        public override Fix64 SolveIteration()
        {
            ...
            if (MovementMode == MovementMode.Floating)
            {
                ...
                accumulatedImpulse.x = MathHelper.Clamp(accumulatedImpulse.x + lambda.x, Fix64.C0, maxForceDt);
