...
namespace FPPhysics.Character
{
    ...
    public class StepManager
    {
        ...
        Fix64 maximumStepHeight = Fix64.C1;
        /// <summary>
        ...
        public Fix64 MinimumDownStepHeight
        {
            ...
            set
            {
                if (minimumDownStepHeight < Fix64.C0)
                    throw new ArgumentException("Value must be nonnegative.");
        ...
        public StepManager(Cylinder characterBody, CharacterContactCategorizer contactCategorizer, SupportFinder supportFinder, QueryManager queryManager, HorizontalMotionConstraint horizontalMotionConstraint)
        {
            ...
            minimumUpStepHeight = CollisionDetectionSettings.AllowedPenetration * Fix64.C1p1;// Math.Max(0, -.01f + character.Body.CollisionInformation.Shape.CollisionMargin * (1 - character.SupportFinder.sinMaximumSlope));
        ...
        public bool TryToStepDown(out Vector3 newPosition)
        {
            ...
            try
            {
                ...
                {
                    currentOffset = hitT + CollisionDetectionSettings.AllowedPenetration * Fix64.C0p5;
                                                ...
                                                out hintOffset))
                    {
                        ...
                        case CharacterContactPositionState.NoHit:
                            ...
                            currentOffset = (lowestBound + currentOffset) * Fix64.C0p5;
                            break;
                        case CharacterContactPositionState.Obstructed:
                            lowestBound = currentOffset;
                            currentOffset = (highestBound + currentOffset) * Fix64.C0p5;
                            break;
                ...
                while (attempts++ < 5 && lowestBound - highestBound > Toolbox.BigEpsilon)
                {
                    ...
                    switch (TryDownStepPosition(ref candidatePosition, ref down, ref tractionContacts, ref supportContacts, ref sideContacts, ref headContacts, out hintOffset))
                    {
                        ...
                        case CharacterContactPositionState.NoHit:
                            highestBound = currentOffset + hintOffset;
                            currentOffset = (lowestBound + highestBound) * Fix64.C0p5;
                            break;
                        case CharacterContactPositionState.Obstructed:
                            lowestBound = currentOffset;
                            currentOffset = (highestBound + lowestBound) * Fix64.C0p5;
                            break;
                        case CharacterContactPositionState.TooDeep:
                            currentOffset += hintOffset;
                            lowestBound = currentOffset;
            ...
            out Fix64 hintOffset)
        {
            ...
            if (tractionContacts.Count > 0)
            {
                ...
                else if (supportState == CharacterContactPositionState.TooDeep)
                {
                    ...
                    hintOffset = MathHelper.Min(Fix64.C0, Fix64.C0p001 - Vector3.Dot(supportContact.Contact.Normal, down) * supportContact.Contact.PenetrationDepth);
                ...
                else //if (supportState == SupportState.Separated)
                {
                    ...
                    hintOffset = -Fix64.C0p001 - Vector3.Dot(supportContact.Contact.Normal, down) * supportContact.Contact.PenetrationDepth;
        ...
        Fix64 upStepMargin = Fix64.C0p1;  //There's a little extra space above the maximum step height to start the obstruction and downcast test rays.  Helps when a step is very close to the max step height.
        void FindUpStepCandidates(RawList<ContactData> outputStepCandidates, ref Vector3 down)
        {
            ...
            foreach (var c in SupportFinder.SideContacts)
            {
                ...
                if (dot > Fix64.C0)
                {
                    ...
                    if (dot < characterBody.Height * Fix64.C0p5 && dot > characterBody.Height * Fix64.C0p5 - maximumStepHeight - upStepMargin)
                    {
                        //It's a candidate!
        ...
        bool TryToStepUsingContact(ref ContactData contact, ref Vector3 down, out Vector3 newPosition)
        {
            ...
            Vector3.Multiply(ref down, characterBody.Height * Fix64.C0p5 - downRayLength, out ray.Position);
            ...
            if (!QueryManager.RayCast(ray, downRayLength, out earliestHit) || //Can't do anything if it didn't hit.
                earliestHit.T <= Fix64.C0 || //Can't do anything if the hit was invalid.
                ...
                if (Toolbox.GetRayPlaneIntersection(ref downRay, ref plane, out hitT, out intersection))
                {
                                              ...
                                              out hintOffset))
                    {
                        case CharacterContactPositionState.Accepted:
                            currentOffset += hintOffset;
                            //Only use the new position location if the movement distance was the right size.
                            if (currentOffset < Fix64.C0 && currentOffset > -maximumStepHeight - CollisionDetectionSettings.AllowedPenetration)
                            {
        ...
        private void PrepareQueryObject(ref Vector3 position)
        {
            ...
            currentQueryObject.UpdateBoundingBoxForTransform(ref transform, Fix64.C0);
        }
            ...
            if (!obstructed && supportContacts.Count > 0)
            {
                ...
                if (supportState == CharacterContactPositionState.Accepted)
                {
                    if (tractionContacts.Count > 0)
                    {
                        ...
                        hintOffset = MathHelper.Min(Fix64.C0, Vector3.Dot(supportContact.Contact.Normal, down) * (CollisionDetectionSettings.AllowedPenetration * Fix64.C0p5 - supportContact.Contact.PenetrationDepth));
                        return CharacterContactPositionState.Accepted;
                    }
                    else
                    {
                        ...
                        Ray downRay;
                        downRay.Position = supportContact.Contact.Position + sideNormal * Fix64.C0p001;
                        Fix64 verticalOffset = Vector3.Dot(downRay.Position - position, down);
                        verticalOffset = characterBody.Height * Fix64.C0p5 + verticalOffset;
                        ...
                        obstructionTestRay.Position = position - down * (characterBody.Height * Fix64.C0p5);
                        obstructionTestRay.Direction = downRay.Position - obstructionTestRay.Position;

                        if (!QueryManager.RayCastHitAnything(obstructionTestRay, Fix64.C1))
                        {
                            ...
                            if (QueryManager.RayCast(downRay, characterBody.Height, out hit))
                            {
                                ...
                                if (characterBody.Height - maximumStepHeight < hit.T)
                                {
                                    ...
                                    if (Fix64.Abs(dot) > ContactCategorizer.TractionThreshold)
                                    {
                                        ...
                                        if (QueryManager.RayCast(downRay, characterBody.Height * Fix64.C0p5 + maximumStepHeight, out hit))
                                        {
                                            //It hit.. almost there!
                                            hit.Normal.Normalize();
                                            Vector3.Dot(ref hit.Normal, ref down, out dot);
