...
namespace FPPhysics.Vehicle
{
    ...
    public class CylinderCastWheelShape : WheelShape
    {
        ...
        public CylinderCastWheelShape(Fix64 radius, Fix64 width, Quaternion localWheelOrientation, Matrix4x4 localGraphicTransform, bool includeSteeringTransformInCast)
        {
        ...
        public override sealed Fix64 Radius
        {
            ...
            set
            {
                shape.Radius = MathHelper.Max(value, Fix64.C0);
                Initialize();
            }
        }
        ...
        public Fix64 Width
        {
            ...
            set
            {
                shape.Height = MathHelper.Max(value, Fix64.C0);
                Initialize();
            }
        }
        ...
        protected internal override void Initialize()
        {
            ...
            if (expansion.x > Fix64.C0)
                boundingBox.Max.x += expansion.x;
            else if (expansion.x < Fix64.C0)
                boundingBox.Min.x += expansion.x;

            if (expansion.y > Fix64.C0)
                boundingBox.Max.y += expansion.y;
            else if (expansion.y < Fix64.C0)
                boundingBox.Min.y += expansion.y;

            if (expansion.z > Fix64.C0)
                boundingBox.Max.z += expansion.z;
            else if (expansion.z < Fix64.C0)
