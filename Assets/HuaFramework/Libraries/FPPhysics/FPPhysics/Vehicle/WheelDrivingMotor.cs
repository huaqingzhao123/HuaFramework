...
namespace FPPhysics.Vehicle
{
    ...
    public class WheelDrivingMotor : ISolverSettings
    {
        ...
        public Fix64 GripFriction
        {
            ...
            set { gripFriction = MathHelper.Max(value, Fix64.C0); }
        }
        ...
        internal Fix64 ApplyImpulse()
        {
            ...
            if (targetSpeed > Fix64.C0)
                accumulatedImpulse = MathHelper.Clamp(accumulatedImpulse, Fix64.C0, maxMotorForceDt); //MathHelper.Min(MathHelper.Max(accumulatedImpulse, 0), myMaxMotorForceDt);
            else if (targetSpeed < Fix64.C0)
                accumulatedImpulse = MathHelper.Clamp(accumulatedImpulse, maxMotorForceDt, Fix64.C0); //MathHelper.Max(MathHelper.Min(accumulatedImpulse, 0), myMaxMotorForceDt);
            else
