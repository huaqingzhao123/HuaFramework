...
namespace FPPhysics.Constraints.TwoEntity.JointLimits
{
    ...
    public class LinearAxisLimit : JointLimit, I1DImpulseConstraintWithError, I1DJacobianConstraint
    {
        ...
        public override Fix64 SolveIteration()
        {
            ...
            if (unadjustedError < Fix64.C0)
                accumulatedImpulse = MathHelper.Min(accumulatedImpulse + lambda, Fix64.C0);
            else
                accumulatedImpulse = MathHelper.Max(accumulatedImpulse + lambda, Fix64.C0);
        ...
        public override void Update(Fix64 dt)
        {
            ...
            //Adjust Error
            if (unadjustedError > Fix64.C0)
                error = MathHelper.Max(Fix64.C0, unadjustedError - margin);
            else if (unadjustedError < Fix64.C0)
                error = MathHelper.Min(Fix64.C0, unadjustedError + margin);
            ...
            springSettings.ComputeErrorReductionAndSoftness(dt, Fix64.C1 / dt, out errorReductionParameter, out softness);
            ...
            if (bounciness > Fix64.C0)
            {
                ...
                Vector3.Dot(ref jAngularB, ref connectionB.angularVelocity, out dot);
                relativeVelocity += dot;
                if (unadjustedError > Fix64.C0 && -relativeVelocity > bounceVelocityThreshold)
                    biasVelocity = MathHelper.Max(biasVelocity, ComputeBounceVelocity(-relativeVelocity));
                else if (unadjustedError < Fix64.C0 && relativeVelocity > bounceVelocityThreshold)
            ...
            if (connectionB.isDynamic)
            {
                ...
                entryB = Fix64.C0;
            massMatrix = Fix64.C1 / (entryA + entryB + softness);
