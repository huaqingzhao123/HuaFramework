...
namespace FPPhysics.CollisionTests.CollisionAlgorithms
{
    ...
    public class TriangleConvexPairTester : TrianglePairTester
    {
        ...
        private bool DoPlaneTest(TriangleShape triangle, out TinyStructList<ContactData> contactList)
        {
            ...
            //Exposes a sqrt but...
            if (t <= Fix64.C0 || distanceSquared < marginSum * marginSum)
            {
                ...
                if (contact.PenetrationDepth > marginSum)
                {
                    ...
                    if (DoExternalNear(triangle, out alternateContacts))
                    {
                        ...
                        if (alternateContact.PenetrationDepth + Fix64.C0p01 < contact.PenetrationDepth) //Bias against the subtest's result, since the plane version will probably have a better position.
                        {
        ...
        private bool DoExternalNear(TriangleShape triangle, out TinyStructList<ContactData> contactList)
        {
            ...
            Vector3.Multiply(ref triangleCentroid, Fix64.OneThird, out triangleCentroid);
            ...
            if (distanceSquared < margin * margin)
            {
                ...
                if (triangle.sidedness != TriangleSidedness.DoubleSided)
                {
                    ...
                    if (triangle.sidedness == TriangleSidedness.Clockwise && dot > Fix64.C0)
                        return false;
                    if (triangle.sidedness == TriangleSidedness.Counterclockwise && dot < Fix64.C0)
                        return false;
                }
        ...
        private bool DoDeepContact(TriangleShape triangle, out TinyStructList<ContactData> contactList)
        {
            ...
            Vector3.Multiply(ref center, Fix64.OneThird, out center);
            ...
            if (MPRToolbox.AreLocalShapesOverlapping(convex, triangle, ref center, ref Toolbox.RigidIdentity))
            {
                ...
                if (lengthSquared < Toolbox.Epsilon * Fix64.C0p01)
                {
                ...
                else
                {
                    ...
                    if ((triangle.sidedness == TriangleSidedness.Clockwise && dot > Fix64.C0) || (triangle.sidedness == TriangleSidedness.Counterclockwise && dot < Fix64.C0))
                    {
        ...
        private bool TryInnerSphereContact(TriangleShape triangle, out ContactData contact)
        {
            ...
            Fix64 minimumRadius = convex.MinimumRadius * (MotionSettings.CoreShapeScaling + Fix64.C0p01);
        ...
        private VoronoiRegion GetVoronoiRegion(TriangleShape triangle, ref Vector3 p)
        {
            ...
            if (APdotAC <= Fix64.C0 && APdotAB <= Fix64.C0)
            {
                //It is A!
            ...
            if (BPdotAB >= Fix64.C0 && BPdotAC <= BPdotAB)
            {
                //It is B!
            ...
            if (vc <= Fix64.C0 && APdotAB > Fix64.C0 && BPdotAB < Fix64.C0) //Note > and < instead of => <=; avoids possibly division by zero
            {
            ...
            if (CPdotAC >= Fix64.C0 && CPdotAB <= CPdotAC)
            {
                //It is C!
            ...
            if (vb <= Fix64.C0 && APdotAC > Fix64.C0 && CPdotAC < Fix64.C0) //Note > instead of >= and < instead of <=; prevents bad denominator
            {
            ...
            if (va <= Fix64.C0 && (BPdotAC - BPdotAB) > Fix64.C0 && (CPdotAB - CPdotAC) > Fix64.C0)//Note > instead of >= and < instead of <=; prevents bad denominator
            {
        ...
        public override VoronoiRegion GetRegion(TriangleShape triangle, ref ContactData contact)
        {
            ...
            Fix64 faceEpsilon = Fix64.C0p01;
            Fix64 edgeEpsilon = Fix64.C0p01;
