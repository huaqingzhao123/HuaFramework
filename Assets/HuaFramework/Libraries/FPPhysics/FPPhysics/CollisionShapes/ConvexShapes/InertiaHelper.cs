...
namespace FPPhysics.CollisionShapes.ConvexShapes
{
    ...
    public static class InertiaHelper
    {
        ...
        public static void GenerateSphere(int subdivisionCount, out Vector3[] vertices, out int[] triangles)
        {
            ...
            var goldenRatio = Fix64.GoldenRatio;
            Fix64 length = Fix64.Sqrt(Fix64.C1 + goldenRatio * goldenRatio);
            Fix64 x = Fix64.C1 / length;
            Fix64 y = goldenRatio / length;
            vertices[0] = new Vector3(Fix64.C0, x, y);
            vertices[1] = new Vector3(Fix64.C0, -x, y);
            vertices[2] = new Vector3(Fix64.C0, x, -y);
            vertices[3] = new Vector3(Fix64.C0, -x, -y);

            vertices[4] = new Vector3(x, y, Fix64.C0);
            vertices[5] = new Vector3(-x, y, Fix64.C0);
            vertices[6] = new Vector3(x, -y, Fix64.C0);
            vertices[7] = new Vector3(-x, -y, Fix64.C0);

            vertices[8] = new Vector3(y, Fix64.C0, x);
            vertices[9] = new Vector3(-y, Fix64.C0, x);
            vertices[10] = new Vector3(y, Fix64.C0, -x);
            vertices[11] = new Vector3(-y, Fix64.C0, -x);
        ...
        public static void ComputeShapeDistribution(IList<Vector3> vertices, IList<int> triangleIndices, out Fix64 volume, out Matrix3x3 volumeDistribution)
        {
            ...
            // [ -b'  b  -a' ]
            // [ -c' -a'  c  ]
            Fix64 a = Fix64.C0, b = Fix64.C0, c = Fix64.C0, ao = Fix64.C0, bo = Fix64.C0, co = Fix64.C0;

            Fix64 scaledVolume = Fix64.C0;
            for (int i = 0; i < triangleIndices.Count; i += 3)
            {
                ...
                ao += scaledTetrahedronVolume * (Fix64.C2 * v2.y * v2.z + v3.y * v2.z + v4.y * v2.z + v2.y * v3.z + Fix64.C2 * v3.y * v3.z + v4.y * v3.z + v2.y * v4.z + v3.y * v4.z + Fix64.C2 * v4.y * v4.z);
                bo += scaledTetrahedronVolume * (Fix64.C2 * v2.x * v2.z + v3.x * v2.z + v4.x * v2.z + v2.x * v3.z + Fix64.C2 * v3.x * v3.z + v4.x * v3.z + v2.x * v4.z + v3.x * v4.z + Fix64.C2 * v4.x * v4.z);
                co += scaledTetrahedronVolume * (Fix64.C2 * v2.x * v2.y + v3.x * v2.y + v4.x * v2.y + v2.x * v3.y + Fix64.C2 * v3.x * v3.y + v4.x * v3.y + v2.x * v4.y + v3.x * v4.y + Fix64.C2 * v4.x * v4.y);
            }
            volume = scaledVolume / Fix64.C6;
            Fix64 scaledDensity = Fix64.C1 / volume;
            Fix64 diagonalFactor = scaledDensity / Fix64.C60;
            Fix64 offFactor = -scaledDensity / Fix64.C120;
            a *= diagonalFactor;
            b *= diagonalFactor;
            c *= diagonalFactor;
            ao *= offFactor;
        ...
        public static void ComputeShapeDistribution(IList<Vector3> vertices, IList<int> triangleIndices, out Vector3 center, out Fix64 volume, out Matrix3x3 volumeDistribution)
        {
            ...
            {
                Vector3.Multiply(ref summedCenter, Fix64.C0p25 / scaledVolume, out center);
                volume = scaledVolume / Fix64.C6;
                Fix64 scaledDensity = Fix64.C1 / volume;
                Fix64 diagonalFactor = scaledDensity / Fix64.C60;
                Fix64 offFactor = -scaledDensity / Fix64.C120;
                ...
                GetPointContribution(Fix64.C1, ref Toolbox.ZeroVector, ref center, out additionalInertia);
