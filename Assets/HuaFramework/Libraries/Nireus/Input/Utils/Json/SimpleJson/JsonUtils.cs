...
public static class JsonUtils
    ...
    public static bool TryFromJson(out object res,Type type,string json)
    {
        ...
        {
            object jsonObj = SimpleJsonTool.DeserializeObject(json);
            //GameDebug.Log("TryFromJson:" + jsonObj);
    ...
    public static bool TryFromJson<T>(out T res, string json)
    {
        try
        {
            object obj = null;
            bool state = TryFromJson(out obj, typeof(T), json);
            res = (obj == null ? default(T) : (T)obj); 
            //GameDebug.Log("Change Value: obj" + obj + " res:" + res);
            return state;
        }
        catch (Exception e)
        {
    ...
    private static object JsonToList(string json, Type itemType)
        ...
        private static object JsonObjectToArray(object data, Type itemType)
        {
            ...
            //GameDebug.Log("JsonObjectToArray : result:" + result.GetType().FullName);
        ...
        private static object ClassOrStructToJsonObject(object data)
        {
            ...
            for (int i = 0; i < propertys.Length; i++)
            {
                ...
                try
                    ...
                    {
                        GameDebug.LogError(type+" property :" + p.Name + "\n" + e);
                        continue;
                    }
                }
            }
            ...
            foreach (var item in dic)
            {
                ...
                else
                {
                    ...
                    if (CheckPropertyInfo(p))
                    {
                        ...
                        {
                            GameDebug.LogError("property :" + p.Name + "\n" + e);
                            continue;
                        }
                    }
                }
        ...
        private static object ChangeJsonDataToObjectByType(Type type, object data)
        {
            ...
            else if (type.IsArray)
            {
                ...
                {
                    GameDebug.LogError("Array无法转换类型， data：" + data.GetType().FullName + "  type.GetElementType(): " + type.GetElementType().FullName);
                    GameDebug.LogError(e);
                }
            }
        ...
        if (value == null)
            ...
            catch (Exception e)
            {
             ...
             //GameDebug.Log("ChangeJsonDataToObjectByType:" + value);
            return value;
        }
