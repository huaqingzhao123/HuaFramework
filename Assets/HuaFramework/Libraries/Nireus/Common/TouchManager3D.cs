...
namespace Nireus
{
    public class TouchManager3D
    {
        ...
        public static Collider[] HitTest(Camera camera, Vector2 screen_pt)
        {
            Ray ray = camera.ScreenPointToRay(screen_pt);
            RaycastHit[] hit = Physics.RaycastAll(ray);
            Collider[] list = new Collider[hit.Length];
            for (int i = 0; i < hit.Length; i++)
            {
                var item = hit[i];
                list[i] = item.collider;
            }
            return list;
        }
        ...
        {
            _ray_target.Clear();
            Collider[] hits = HitTest(camera, screen_pt);
